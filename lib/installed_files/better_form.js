...
function validateField() {
  ...
  if ($(this).is(':text, textarea, :password') && (this.value == this.defaultValue)) {
	...
	ajax_url = ajax_url + '/ajax_validate_' + method

  $.ajax({data:'authenticity_token=' + encodeURIComponent('iH4oAzNSO8OUiyFjVksfIdjEzmiWbL5BfM3mgp4rws4=')+ '&' + this.name + '=' + this.value + '&field_id=' + this.id, dataType:'script', type:'post', url: ajax_url, async: false});
}

function checkFormIsCompleted(e) {
  $.ajax({data: $(this).serialize() + '&field_id=' + this.id, dataType:'script', type:'post', url: 'ajax_validate_' + this.id, async: false});
	if (validForm == true) {
		return true;
	} else {
		return false;
