...
function validateField(field) {
  ...
  $.ajax({data:'authenticity_token=' + getAuthenticityToken() + '&model=' + field_model_name + '&attribute=' + field_attribute_name + '&' + field_model_name + '[' + field_attribute_name + ']' + '=' + field.value + '&field_id=' + field.id, dataType:'script', type:'post', url: ajax_url, async: true});
}
...
function checkFormIsCompleted() {
	...
	if ($(this).find(':input.better_validated_field').not('.better_valid_field').size() > 0) {
		return false;
	} else {
		return true;
	}
